...
var JsImage, b_exp, camera, colorify, colormap, colormap1, createBuffer, createContext, createTexture, drawScene, g_exp, generateShader, getURLParameter, get_channels, glHandleOnClickColor, glHandleOnClickDownload, glHandleOnClickGrey, glHandleOnClickNdvi, glHandleOnClickRaw, glHandleOnClickSave, glHandleOnLoadTexture, glHandleOnSlide, glHandleOnSubmitInfra, glHandleOnSubmitInfraHsv, glHandleOnSubmitInfraMono, glInitInfragram, glRestoreContext, glSetMode, glShaderLoaded, glUpdateImage, greyscale_colormap, histogram, hsv2rgb, image, imgContext, infragrammar, jsHandleOnClickColor, jsHandleOnClickDownload, jsHandleOnClickGrey, jsHandleOnClickNdvi, jsHandleOnClickRaw, jsHandleOnClickSave, jsHandleOnSlide, jsHandleOnSubmitInfra, jsHandleOnSubmitInfraHsv, jsHandleOnSubmitInfraMono, jsUpdateImage, last_command, log, m_exp, mapContext, mode, modeToEquationMap, ndvi, params, r_exp, render, rgb2hsv, save_expressions, save_expressions_hsv, segmented_colormap, setParametersFromURL, set_mode, update, updateImage, update_colorbar, vertices, waitForShadersToLoad, webGlSupported,
  _this = this;
...
jsHandleOnClickSave = function() {
  var ctx, data, e, orig_data, thumb_data;
  e = $("#image")[0];
  ctx = e.getContext("2d");
  ...
  ctx.canvas.height = parseInt((240 * image.height) / image.width);
...
$(document).ready(function() {
  ...
  if (location.toString().split('?')[1]) {
    $.each(location.toString().split('?')[1].split('&'), function(i, o) {
      var k, v;
      k = o.split('=')[0];
      v = o.split('=')[1];
      return params[k] = v;
