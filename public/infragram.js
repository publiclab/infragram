...
var JsImage, activeFile, b_exp, camera, colorify, colormap, colormap1, createBuffer, createContext, createTexture, drawScene, filename, g_exp, generateShader, getFilename, getURLParameter, get_channels, glGetCurrentImage, glHandleOnClickColor, glHandleOnClickGrey, glHandleOnClickNdvi, glHandleOnClickRaw, glHandleOnSlide, glHandleOnSubmitInfra, glHandleOnSubmitInfraHsv, glHandleOnSubmitInfraMono, glInitInfragram, glRestoreContext, glSetMode, glShaderLoaded, glUpdateImage, greyscale_colormap, handleOnChangeFile, histogram, hsv2rgb, image, imgContext, infragrammar, jsGetCurrentImage, jsHandleOnClickColor, jsHandleOnClickGrey, jsHandleOnClickNdvi, jsHandleOnClickRaw, jsHandleOnSlide, jsHandleOnSubmitInfra, jsHandleOnSubmitInfraHsv, jsHandleOnSubmitInfraMono, jsUpdateImage, loadFileFromUrl, log, m_exp, mapContext, mode, modeToEquationMap, ndvi, r_exp, render, rgb2hsv, save_expressions, save_expressions_hsv, segmented_colormap, sendThumbnail, setParametersFromURL, set_mode, socket, update, updateImage, update_colorbar, upload, vertices, waitForShadersToLoad, webGlSupported,
  _this = this,
...
handleOnChangeFile = function(files, onLoadImage) {
  ...
  if (files && files[0]) {
    ...
    upload.onload = function(event) {
      return socket.emit("image_send", {
        "id": socket.socket.sessionid,
        "data": event.target.result
      });
    };
    socket.emit("image_start", {
      "id": socket.socket.sessionid,
    ...
    return reader.readAsDataURL(activeFile);
  }
};

socket.on("image_request", function(data) {
...
socket.on("image_done", function(data) {
  var txt;
  if (__indexOf.call(data, "error") >= 0) {
    alert(data["error"]);
  } else {
    filename = data["name"];
  }
...
loadFileFromUrl = function(url, onLoadImage) {
  var funTxt, name;
  name = url.substring(url.lastIndexOf("/") + 1);
  funTxt = onLoadImage.toString();
  return socket.emit("url", {
    "name": name,
    "url": url,
    "on_load": funTxt
  });
...
socket.on("done_url", function(data) {
  var img;
  filename = data["name"];
  img = new Image();
  img.onload = function() {
