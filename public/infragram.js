...
var FileUpload, JsImage, b_exp, camera, colorify, colorized, colormap, colormap1, colormap2, createBuffer, createContext, createTexture, decodeParameters, downloadImage, drawScene, expressions, fetch_image, g_exp, generateShader, getCurrentImage, getURLParameter, get_channels, glGetCurrentImage, glHandleOnClickColor, glHandleOnClickGrey, glHandleOnClickNdvi, glHandleOnClickRaw, glHandleOnSlide, glHandleOnSubmitInfra, glHandleOnSubmitInfraHsv, glHandleOnSubmitInfraMono, glInitInfragram, glRestoreContext, glRunInfragram, glSetMode, glShaderLoaded, glUpdateImage, greyscale_colormap, histogram, hsv2rgb, image, imgContext, infragrammar, infragrammar_mono, jsGetCurrentImage, jsHandleOnClickGrey, jsHandleOnClickNdvi, jsHandleOnClickRaw, jsHandleOnSlide, jsHandleOnSubmitInfra, jsHandleOnSubmitInfraHsv, jsHandleOnSubmitInfraMono, jsRunInfragrammar, jsUpdateImage, log, log_hsv, log_mono, log_rgb, m_exp, mapContext, mode, modeToEquationMap, ndvi, parametersObject, params, preset_ndvi_blue, preset_ndvi_blue_color, preset_ndvi_red, preset_ndvi_red_color, preset_raw, r_exp, render, rgb2hsv, run_colorize, run_infragrammar, save_expressions, save_expressions_hsv, save_infragrammar_expressions, save_infragrammar_inputs, save_log, segmented_colormap, setParametersFromURL, set_mode, update, updateImage, update_colorbar, vertices, video_live, waitForShadersToLoad, webGlSupported,
  _this = this;
...
glRunInfragram = function(mode) {
  return glSetMode(imgContext, mode);
};
...
run_colorize = function() {
  render(colorify(infragrammar_mono(image), function(x) {
...
preset_ndvi_red_color = function() {
  $('#modeSwitcher').val("infragrammar_mono").change();
  $('#m_exp').val("(B-R)/(B+R)");
  $('#preset-modal').modal('hide');
  save_infragrammar_inputs();
...
preset_ndvi_blue_color = function() {
  $('#modeSwitcher').val("infragrammar_mono").change();
  $('#m_exp').val("(R-B)/(R+B)");
  $('#preset-modal').modal('hide');
  save_infragrammar_inputs();
...
$(document).ready(function() {
  ...
  $("#image-container").ready(function() {
    ...
    enablewebgl = getURLParameter("enablewebgl") === "true" || getURLParameter("webgl") === "true" ? true : false;
